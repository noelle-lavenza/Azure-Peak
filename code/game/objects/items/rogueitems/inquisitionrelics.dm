		...
		if(istype(W, /obj/item/roguekey/psydonkey))
			...
			switch(relicchoice)
				...
				if("Stigmata - Silver Halberd")
					...
					user.mind?.adjust_skillrank_up_to(/datum/skill/combat/polearms, SKILL_LEVEL_EXPERT, TRUE)	//We make sure the weapon is usable by the Inquisitor.
				if("Apocrypha - Silver Greatsword")
					choice = /obj/item/rogueweapon/greatsword/psygsword
					user.mind?.adjust_skillrank_up_to(/datum/skill/combat/swords, SKILL_LEVEL_EXPERT, TRUE)		//Ditto.
