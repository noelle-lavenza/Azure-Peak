	...
	if(reagents)
		if(reagents.flags & TRANSPARENT)
			if(length(reagents.reagent_list))
				if(user.can_see_reagents() || (user.Adjacent(src) && (user.mind.get_skill_level(/datum/skill/craft/alchemy) >= SKILL_LEVEL_APPRENTICE || HAS_TRAIT(user, TRAIT_CICERONE)))) //Show each individual reagent
					. += "It contains:"
