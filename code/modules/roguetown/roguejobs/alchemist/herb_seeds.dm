...
/obj/item/herbseed/examine(mob/user)
	...
	if(isliving(user))
		...
		else if(living.mind.get_skill_level(/datum/skill/craft/alchemy) >= SKILL_LEVEL_APPRENTICE || living.mind.get_skill_level(/datum/skill/labor/farming) >= SKILL_LEVEL_APPRENTICE)
			show_real_identity = TRUE
	else
		show_real_identity = TRUE
